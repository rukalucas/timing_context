...
class BaseTrainer:
    ...
    def __init__(
        ...
        if optimizer_type.lower() == "adam":
            ...
            raise ValueError(
                f"Unknown optimizer_type: {optimizer_type}. Use 'adam', 'adamw', or 'sgd'."
            )
        ...
        if config and "wandb" in config:
            ...
            if "SLURM_JOB_ID" in os.environ:
                ...
                if resume and "slurm_jobs" in wandb.config:
                    ...
                    wandb.config.update(
                        {"slurm_jobs": existing_jobs}, allow_val_change=True
                    )
                else:
                    # Fresh run or first SLURM job - create list
                    wandb.config.update(
                        {"slurm_jobs": [slurm_info]}, allow_val_change=True
                    )
    ...
    def _forward_pass(
        self, batch, task, include_iti: bool = False, return_hidden: bool = False
    ) -> tuple:
        ...
        for trial_idx, trial in enumerate(batch):
            ...
            if (
                include_iti
                and trial_idx < N - 1
                and hasattr(task, "_generate_iti_inputs")
            ):
                ...
                if return_hidden:
                    ...
                    all_iti_outputs.append(
                        torch.stack(iti_outputs, dim=1)
                    )  # [B, iti_len, 2]
        ...
        with torch.no_grad():
            ...
            if log_figures and hasattr(task, "_generate_iti_inputs"):
                (
                    ...
                    eval_batch, task, include_iti=True, return_hidden=True
                )
            else:
                all_outputs, all_inputs, all_targets, all_eval_masks, all_loss_masks = (
                    self._forward_pass(eval_batch, task)
                )
            ...
            if log_figures:
                ...
                for trial_idx in range(N):
                    ...
                    eval_mask_np = all_eval_masks[trial_idx][
                        batch_idx
                    ].numpy()  # [T, 2]
                    loss_mask_np = all_loss_masks[trial_idx][
                        batch_idx
                    ].numpy()  # [T, 2]
                    ...
                    iti_inputs_np = None
                    iti_outputs_np = None
                    if hasattr(task, "_generate_iti_inputs") and trial_idx < len(
                        all_iti_inputs
                    ):
                        iti_inputs_np = all_iti_inputs[trial_idx][
                            batch_idx
                        ].numpy()  # [iti_len, 5]
                        iti_outputs_np = all_iti_outputs[trial_idx][
                            batch_idx
                        ].numpy()  # [iti_len, 2]
                    ...
                    figure_name = (
                        f"{task_name}/trial_{trial_idx + 1}"
                        if task_name
                        else f"trial_{trial_idx + 1}"
                    )
    ...
    def load_checkpoint(
        ...
        if reset_counters:
            print(
                f"Loaded checkpoint from {checkpoint_path} as initialization (step counters reset)"
            )
            self.step = 0
        else:
            self.step = checkpoint["step"]
            print(
                f"Loaded checkpoint from {checkpoint_path} (resuming from step {self.step})"
            )
        ...
        extra_state = {
            ...
            if k
            not in ["step", "model_state_dict", "optimizer_state_dict", "wandb_run_id"]
        }
